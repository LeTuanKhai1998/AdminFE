    ...
    <div class="card shadow" style="margin-top: 100px"
        ...
        <modal :show.sync="isModal">
            ...
            <div v-if="selectedItem">
                ...
                <base-input alternative="" label="Mô tả">
                    <textarea spellcheck="false" rows="4" class="form-control form-control-alternative"
                              placeholder="Viết vài điều về đạo diễn ..."
            ...
            <template slot="footer">
                ...
                <base-button type="primary" @click.prevent="updateDirector">Lưu thay đổi</base-button>
            </template>
        </modal>
        <modal :show.sync="isDeleteModal">
            ...
            <div v-if="selectedItem">
                <h4 class="modal-title">Bạn chắc chắn muốn xóa đạo diễn "{{this.selectedItem.name}}" không?</h4>
                <h5 class="modal-title" style="color: red;margin-top: 5px">(Lưu ý xóa đạo diễn sẽ xóa luôn dữ liệu
                    người đó)</h5>
            </div>
            <!--            </div>-->
            <template slot="footer">
                <base-button type="secondary" @click.prevent="isDeleteModal = false">Bỏ qua</base-button>
                <base-button type="primary" @click.prevent="deleteDirector">Xác nhận xóa</base-button>
            </template>
        </modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteDirector() {
                MovieService.deleteDirectorById(this.selectedItem.id)
                    .then(response => {
            ...
            updateDirector() {
                ...
                MovieService.updateDirector(this.selectedItem)
                    .then(response => {
                        this.message = response.data.data.data;
                        if (this.message.localeCompare("successful") == 0) {
            ...
            prepation() {
                if ((this.pagination.currentPage == 0 && this.tableData.length == 0) || this.pagination.currentPage != 0) {
                    MovieService.getAllDirectors(this.pagination)
                        .then(response => {
